...
export type DocumentStatus = 'Draft' | 'Pending Review' | 'Approved' | 'Archived'
export type DocumentCategory = 'Policy' | 'Procedure' | 'Contract' | 'Training' | 'Report'
...
export const handlers = [
  ...
  http.post('/documents/import', async ({ request }) => {
    const payload = await request.json() as Array<Record<string, string | undefined>>
    const docs = readDocuments()
    ...
    payload.forEach((row, index) => {
      ...
      const validCategory = ['Policy', 'Procedure', 'Contract', 'Training', 'Report'].includes(category)
      const validStatus = ['Draft', 'Pending Review', 'Approved', 'Archived'].includes(status)
      if (!code || !title || !validCategory || !validStatus || code.length < 3 || title.length < 4) {
        invalidRows.push({ row: index + 1, reason: 'Missing or invalid code/title/category/status' })
        return
      }
      docs.unshift({
